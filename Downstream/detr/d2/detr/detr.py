...
class Detr(nn.Module):
    ...
    def __init__(self, cfg, args=None):
        ...
        #load pimae weights
        if cfg.MODEL.PIMAE.load_pretrain:
            
            weight = torch.load(cfg.MODEL.PIMAE.load_r50, map_location="cpu")['model']
            new_dict = {}
            #remove resnet weights
            for k in weight.keys():
                if "backbone" in k:
                    new_dict[k] = weight[k]
                    
            
            weight = torch.load(cfg.MODEL.PIMAE.load_pretrain, map_location="cpu")['model']
            #remove resnet weights
            for k in weight.keys():
                if "backbone" not in k:
                    new_dict[k] = weight[k]
            del new_dict["class_embed.weight"]         
            del new_dict["class_embed.bias"] 
            ...
            for k in new_dict.keys():
                save_dict["detr."+ k ] = new_dict[k]
            #print(new_dict.keys())
            
                
            torch.save({"model": save_dict}, "converted_pimae.pth")
            self.detr.load_state_dict(new_dict, strict=False)
