...
DATA_PATH_V1 = "../data/sunrgbd_local/sunrgbd_pc_bbox_votes_50k_v1"  ## Replace with path to dataset
...
class SunrgbdDetectionDataset(Dataset):
    def __init__(
        ...
        # subset
        if subset > 0:
            tot_len = len(self.scan_names)
            print(tot_len)
            samples = np.random.random_sample(tot_len) * subset
            samp_idx = [samples < 1]
            self.scan_names = np.array(self.scan_names)[tuple(samp_idx)].tolist()
    ...
    def __getitem__(self, idx):
        ret_dict = self.get_target(idx)
    
        is_empty = False
        max_len = len(self.scan_names)
        gt_num = ret_dict["gt_box_present"].sum()
        cnt = 0
        while gt_num <= 0:
            new_idx = random.randint(0, max_len-1)
            ret_dict = self.get_target(new_idx)
            gt_num = ret_dict["gt_box_present"].sum()
            cnt += 1
            if cnt >= 10:
                break
