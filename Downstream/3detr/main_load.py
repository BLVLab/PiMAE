...
def make_args_parser():
    ...
    parser.add_argument("--encoder_learning_rate", default=5e-5, type=float)
    ...
    parser.add_argument("--subset", required=True, default=0.0, type=float)
    parser.add_argument(
        "--dataset_root_dir",
        type=str,
        default=None,
    ...
    parser.add_argument("--specific_encoder_depth", default=0, type=int)
    parser.add_argument("--joint_encoder_depth", default=0, type=int)
...
def do_train(
    ...
    if is_primary() and not args.no_wandb:
        # pass
        run = wandb.init(project='3detr', entity="your-entity",sync_tensorboard=True, config=args)
         
    logger = Logger(args.checkpoint_dir)

    if args.unfreeze_epoch:
        freeze(model, args.unfreeze_epoch)
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        if (
            ...
            print(f'saved resuming checkpoint at epoch {epoch}')

        if epoch >= 800 and (epoch % args.eval_every_epoch == 0 or epoch == (args.max_epoch - 1)):
            ...
            print(f'saved evaluation checkpoint at epoch {epoch}')
...
def loadPretrain(model,path,first_depth,second_depth, is_distributed=False):
    ...
    # second_depth - joint encoder depth
    ...
    for i in range(0,first_depth):
        for name in weights_toload:
            if is_distributed:
                dict[f"encoder.layers.{i}.{name}"] = f"module.pc_branch.MAE_encoder.blocks.layers.{i}.{name}"
            else:
                dict[f"encoder.layers.{i}.{name}"] = f"pc_branch.MAE_encoder.blocks.layers.{i}.{name}"

    for i in range(first_depth,total_depth):
        j = i - first_depth
        for name in weights_toload:
            if is_distributed:
                dict[f"encoder.layers.{i}.{name}"] = f"module.blocks.layers.{j}.{name}"
            else:
                dict[f"encoder.layers.{i}.{name}"] = f"module.blocks.layers.{j}.{name}"
...
def main(local_rank, args):
    ...
    # wandb init in rank 0
    # if local_rank == 0:
    #  run = wandb.init(project='finetune', entity="mmp-mae",sync_tensorboard=True, config=args)
    ...
    if args.load_pretrain:
        ...
        try:
            loadPretrain(model, pretrain_path, specific_encoder_depth, joint_encoder_depth)
        except:
            loadPretrain(model, pretrain_path, specific_encoder_depth, joint_encoder_depth, True)
